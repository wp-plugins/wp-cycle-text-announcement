...
Tags: Wordpress, plugin, cycle, announcement, text
*/
...
add_filter('the_content','wpcytxt_show_filter');

function wpcytxt_show_filter($content)
{
	return 	preg_replace_callback('/\[CYCLE-TEXT=(.*?)\]/sim','wpcytxt_show_filter_callback',$content);
}

function wpcytxt_show_filter_callback($matches) 
{
	global $wpdb;
	$setting = $matches[1];
	$wpcycle = "";
...
add_action('init', 'wpcytxt_add_javascript_files');
